...
export function ComparisonTab({
  versions, versionA, versionB, onChangeA, onChangeB, data, onCompare,
}: {
  ...
  return (
    <Card title="Version Comparison" icon="layers" sub="Compare metrics between two model versions">
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 16 }}>
        ...
        <button className="btn btn-sm btn-primary" onClick={onCompare} disabled={!versionA || !versionB}>Compare</button>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            {["mae", "rmse", "mape", "r2_score", "performance_ratio"].map((metric) => {
              const a = (data.versions[0] as Record<string, unknown>)[metric] as number | null;
              const b = (data.versions[1] as Record<string, unknown>)[metric] as number | null;
